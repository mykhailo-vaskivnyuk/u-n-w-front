...
export const api = (fetch: <T>(pathname: string, options?: Record<string, any>) => Promise<T>) => ({
  account: {
    ...
    overmail: (options: Types.TAccountOvermail) =>
      fetch<Types.TAccountOvermailResponse>('/account/overmail', options),
  ...
  user: {
    create: (options: Types.TUserCreate) =>
      fetch<Types.TUserCreateResponse>('/user/create', options),
